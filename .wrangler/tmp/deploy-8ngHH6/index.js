...
function isRetryableContainerError(message) {
  return message.includes("not running") || message.includes("no container instance") || message.includes("try again later") || message.includes("network connection lost") || message.includes("operation was aborted") || message.includes("durable object reset") || message.includes("its code was updated") || message.includes("connection reset") || message.includes("econnreset");
}
__name(isRetryableContainerError, "isRetryableContainerError");
async function wait(ms) {
  await new Promise((resolve) => setTimeout(resolve, ms));
}
__name(wait, "wait");
async function fetchFromContainer(request, env) {
  const container = getContainer(env.API_CONTAINER, "api");
  const retryDelaysMs = [500, 1200, 2500, 4e3];
...
__name(fetchFromContainer, "fetchFromContainer");
